...
Napi::Object ELFBinary::Init(Napi::Env env, Napi::Object exports) {
  Napi::Function constructor = DefineClass(env, "Binary", {
    InstanceMethod<&ELFBinary::GetSection>("get_section"),
...
ELFBinary::ELFBinary(const Napi::CallbackInfo& info)
    : Napi::ObjectWrap<ELFBinary>(info), binary_(nullptr) {
  Napi::Env env = info.Env();

  if (info.Length() < 1) {
    Napi::TypeError::New(env, "ELFBinary constructor requires a file path").ThrowAsJavaScriptException();
    return;
  }
...
Napi::Value ELFBinary::Write(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
